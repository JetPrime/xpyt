"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var core_1 = require("@angular/core");
var youtube_service_1 = require("../../services/youtube.service");
var ChannelComponent = (function () {
    function ChannelComponent(ytService) {
        this.ytService = ytService;
    }
    ChannelComponent.prototype.ngOnChanges = function () {
        if (this.channelRequest)
            this.getChannel();
    };
    ;
    ChannelComponent.prototype.getChannel = function () {
        var _this = this;
        this.ytService.getChannel(this.channelRequest).then(function (channel) {
            _this.channel = channel[0];
            _this.getPlaylists(channel[0].id);
        });
    };
    ;
    ChannelComponent.prototype.getPlaylists = function (pId) {
        var _this = this;
        this.ytService.getPlaylists(pId).then(function (playlists) { return _this.playlists = playlists; });
    };
    ;
    return ChannelComponent;
}());
__decorate([
    core_1.Input(),
    __metadata("design:type", String)
], ChannelComponent.prototype, "channelRequest", void 0);
ChannelComponent = __decorate([
    core_1.Component({
        selector: 'cmp-channel',
        templateUrl: './channel.component.html',
        styleUrls: ['../../../styles/app.component.css']
    }),
    __metadata("design:paramtypes", [youtube_service_1.YoutubeService])
], ChannelComponent);
exports.ChannelComponent = ChannelComponent;
//# sourceMappingURL=channel.component.js.map